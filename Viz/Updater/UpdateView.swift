    ...
    var body: some View {
        VStack(spacing: 5) {
            ...
            if !isAppInCorrectDirectory {
                Text("To avoid permission issues, please move Viz to the \(isUserAdmin ? "/Applications" : "\(NSHomeDirectory())/Applications") folder before updating!")
                    .font(.callout)
...
struct NoUpdateView: View {
    ...
    var body: some View {
        VStack(spacing: 5) {
            ...
            Text("Viz is on the most current release available, but you may force a re-download of the same version below.")
                .font(.body)
                .lineLimit(3)
                .multilineTextAlignment(.leading)
                .padding()
